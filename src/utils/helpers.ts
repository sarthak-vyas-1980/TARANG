...
import { type HazardType, type SeverityLevel, type ReportStatus, type SentimentType } from '../types';
...
/**
 * Format hazard type to display label
 */
export const formatHazardType = (hazardType: HazardType): string => {
  return HAZARD_TYPE_LABELS[hazardType] || hazardType.replace('_', ' ');
};

/**
 * Get severity styling
 */
export const getSeverityStyles = (severity: SeverityLevel) => {
...
export const getStatusStyles = (status: ReportStatus) => {
  return REPORT_STATUS_COLORS[status] || REPORT_STATUS_COLORS.pending;
};
